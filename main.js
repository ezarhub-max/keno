...
// TEMPORARY: Re-add a deleted user
app.get('/admin/restore-user/:phone', async (req, res) => {
    ...
    try {
        ...
        const referralCode = `REF${phone.slice(-8)}${randomNum}`;
        
        // Insert new user
        const result = await pool.query(
            `INSERT INTO users (phone, password, wallet_balance, referral_code, created_at) 
             VALUES ($1, $2, $3, $4, NOW()) 
             RETURNING id, phone, wallet_balance, referral_code, created_at`,
            [phone, 'telegram123', 5210.00, referralCode]
        );
        
        console.log(`✅ User restored: ${phone} with balance $0.00`);
